...
public class PDFGenerator {
    ...
    /**
     * Genera un reporte PDF de usuarios (Solo para Admin)
     */
    public static void generarReporteUsuarios(List<Usuario> usuarios, String rutaArchivo)
            throws DocumentException, IOException {

        Document documento = new Document(PageSize.A4.rotate()); // Horizontal para que quepan los datos
        PdfWriter.getInstance(documento, new FileOutputStream(rutaArchivo));
        documento.open();

        // Título
        Paragraph titulo = new Paragraph("REPORTE DE USUARIOS - SISTEMA LICENCIAS", FONT_TITULO);
        titulo.setAlignment(Element.ALIGN_CENTER);
        documento.add(titulo);
        documento.add(Chunk.NEWLINE);
        ...
        tabla.setWidthPercentage(100);
        // Anchos relativos aproximados
        tabla.setWidths(new float[]{30f, 15f, 20f, 20f, 20f, 35f, 20f, 20f});

        // Encabezados
        String[] headers = {"Nombre", "ID", "Usuario", "Clave", "Cédula", "Email", "Fecha", "Rol"};
        for (String header : headers) {
            PdfPCell celda = new PdfPCell(new Phrase(header, FONT_CAMPO));
        ...
        for (Usuario u : usuarios) {
            agregarCeldaTabla(tabla, u.getNombreCompleto());
            agregarCeldaTabla(tabla, String.valueOf(u.getId()));
            agregarCeldaTabla(tabla, u.getUsuario());
            agregarCeldaTabla(tabla, u.getClave()); // Mostrar contraseña explícitamente
            agregarCeldaTabla(tabla, u.getCedula());
            agregarCeldaTabla(tabla, u.getEmail() != null ? u.getEmail() : "-");
            agregarCeldaTabla(tabla, u.getFechaCreacion().format(fechaCorta));
            agregarCeldaTabla(tabla, u.getRol());
        }

        documento.add(tabla);

        // Pie
        Paragraph pie = new Paragraph("Total registros: " + usuarios.size(), FONT_NORMAL);
    ...
    private static void agregarCeldaTabla(PdfPTable tabla, String texto) {
        PdfPCell celda = new PdfPCell(new Phrase(texto, FontFactory.getFont(FontFactory.HELVETICA, 9)));
        tabla.addCell(celda);
    }
}
