...
public class LicenciaDAO implements Persistible<Licencia> {
    ...
    public List<Licencia> obtenerLicenciasVigentes() throws BaseDatosException {
        // Cambio: CURDATE() por CURRENT_DATE (Sintaxis Postgres)
        String sql = "SELECT * FROM licencias WHERE activa = TRUE AND fecha_vencimiento > CURRENT_DATE " +
        ...
        try (Connection conn = dbConfig.obtenerConexion();
            ...
            throw new BaseDatosException("Error al consultar vigencia en la nube: " + e.getMessage(), e);
        }
    }
